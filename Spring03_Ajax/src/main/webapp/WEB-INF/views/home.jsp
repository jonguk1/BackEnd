	...
	<script>
		$(function(){
			$('a[href="newRss1"]').click(function(e){
				e.preventDefault();//기본 동작을 막는다.(a태그의 경우 href 지정된 url이동하려함==>이를 막는다)
				//alert('a');
				$.ajax({
					...
					.done(function(data){
						...
						$(data.items).each(function(i, item){
							...
							$.ajax({
								type:'post',
								url:'news_dbCreate',
								async:false,//==>false주면 동기적 방식으로 요청을 보낸다 (순서를 지켜서)
								data:{
									title:title,
									link,link,
									pubDate:pubDate
...
<a href="newRss3"><h3>오늘의 뉴스3(json으로 받아온뒤 DB에 저장)</h3></a>
