				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="row">
							...
							<!-- .col -->
							<!-- 리뷰 목록 xl,lg,md(desktop),sm(tablet), xs(mobile) -->
							<div class="col-md-6 col-sm-12-col-xs-12">
								<ul id="reviewList">
<!-- 									<li>
									<img src="resources/image/noimage.PNG" class="img img-thumbnail">
									</li>
									<li>
...
<script>
	...
	function send(){
		...
		//alert(btnTxt);
		//[1] 파일을 업로드 하지 않을 경우. input값들을 파라미터 문자열로 만들어서 전송해야 함 title=aaaa&content=bbb&score=5
 	...
 	function writeReviewAndFile(formData){
 		$.ajax({
 			...
 			processData:false,//일반적으로 서버에 전달하는 데이터는 query string형태, false를 주면 query string을 사용
 			cache:false,
 	...
 	//PUT /reviews ==>수정처리(파일 업로드할떄는 POST로 처리하자)
	...
	function showList(res){//res==>배열
		...
		$(res).each(function(i, obj){
			...
			if(obj.filename==null){
				str+=`<li><img src="resources/image/noimage.PNG" class="img img-thumbnail">`;
			}else{
				str+=`<li><img src="resources/image/\${obj.filename}" class="img img-thumbnail">`;
			}
	...
	function edit(no){
		...
		.done((res)=>{
			...
			if(res.filename!=null){
				let str=`<img src="resources/image/\${res.filename}" class="img-fluid">`;
				$('#prodImg').html(str);
			}
