...
public class JobKorea {
	...
	public void process() {
		...
		while(true){
			...
			case 7:
				...
				String delteResult = delete(deleteName);
				System.out.println(delteResult);
				break;
			default:
				break;
			}
	...
	/**이름으로 배열에서 해당 이름의 구직자 정보를 찾아 수정하고 문자열을 반환하는 메서드
	 * */
	...
	/**이름으로 배열에서 해당 이름의 구직자 정보를 찾아 삭제하고 문자열을 반환하는 메서드
	 * */
	public String delete(String deleteName) {
		...
		for(int i=0;i<count;i++) {
			if(persons[i].getName().equals(deleteName)) {
				...
				count--;
				str += deleteName + "님의 정보를 삭제하였습니다. ";
				str += "현재 등록인원은 " + count + "명 입니다";
			}
		}
		if(str.equals("")) {
			return deleteName+"님 점보는 없습니다";
		}
		return str;
	}
